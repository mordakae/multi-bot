...
The default target is `~/.codex/AGENTS.md` (Codex's user-global instructions).
...
- **Back up user prefs before making any changes:**
  - Read the existing `~/.codex/AGENTS.md` (if it exists)
...
confirm Codex's **current** conventions for the surfaces this sync touches — where user-global
`AGENTS.md` instructions, skills, agents, and MCP config are read from, and how each is structured —
from the platform's current documentation, using any config already present on the system as a clue.
...
User-level config is **global** — it applies in every repo, so per-folder scoping does not apply.
...
- Apply the same `description` rules as for rules (above)
...
After writing all agent-config-owned sections to `~/.codex/AGENTS.md`:
...
- In `~/.codex/AGENTS.md`, check that every agent-config-owned heading has a corresponding file
  in `~/.agent_config`
