...
The default target for user-global rules is `~/.gemini/GEMINI.md`.
...
confirm Gemini's **current** conventions for the surfaces this sync touches — where user-global
rules, skills, personas, and MCP config are read from, and which frontmatter fields each supports —
...
User-level config is **global** — it applies in every directory tree, so per-folder scoping does
not apply. For each file in `~/.agent_config/rules`:
- Write inside the agent-config sync markers in `~/.gemini/GEMINI.md`
- If `globs` contains file types, add a comment at the top of the rule block:
