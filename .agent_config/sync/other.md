...
- Identify the platform's existing config file(s) by looking for common conventions
  (e.g. `.platform/config`, `PLATFORM.md`, `platform.json` in the repo root or a config dir)
...
- If MCP integration is supported, integrate `.agent_config/mcp.json` into the platform's
  config format
- Copy `.agent_config/agent_config_version` to the platform's config directory as
